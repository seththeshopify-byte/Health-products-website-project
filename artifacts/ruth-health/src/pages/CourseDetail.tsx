...
function ModuleQuiz({
  ...
  const { data: questions, isLoading } = useQuery<QuizQuestion[]>({
    ...
    queryFn: () => authFetch(`/courses/${courseId}/modules/${moduleId}/quiz`),
    enabled: !!moduleId,
  });

  const { data: existingResult } = useQuery<QuizResult>({
    queryKey: ["moduleQuizResult", courseId, moduleId],
    queryFn: () => authFetch(`/courses/${courseId}/modules/${moduleId}/quiz/result`),
    enabled: !!moduleId,
  });

  const submitMutation = useMutation({
    mutationFn: (answers: Array<{ questionId: number; selectedOptionIndex: number }>) =>
      authFetch(`/courses/${courseId}/modules/${moduleId}/quiz/submit`, {
        method: "POST",
...
export default function CourseDetail() {
  ...
  const { data: modules, isLoading: modulesLoading } = useQuery<CourseModule[]>({
    ...
    queryFn: () => authFetch(`/courses/${id}/modules`),
    enabled: !!id,
  });
    ...
    queryFn: async () => {
      ...
      await Promise.all(
        modules.map(async (mod) => {
          try {
            const res = await authFetch(`/courses/${id}/modules/${mod.id}/quiz/result`);
            results[mod.id] = res;
          } catch {
  ...
  const allModulesPassed = modules && modules.length > 0 && modules.every((m) => passedModules.has(m.id));
  ...
  // Legacy: course has no modules, use old contentBody behavior
        ...
        {!course.isEnrolled ? (
          <div className="bg-card border rounded-2xl overflow-hidden shadow-sm">
            <div className="aspect-video relative bg-muted group cursor-pointer" onClick={handleEnroll}>
              <AppImage src={course.imageUrl || undefined} fallbackType="course" alt={course.name} className="w-full h-full object-cover opacity-80" />
            ...
            <div className="p-8 text-center max-w-lg mx-auto">
              ...
              <p className="text-muted-foreground mb-8">This educational resource is available for free to all members. Create an account or sign in to start learning.</p>
              <Button onClick={handleEnroll} disabled={enrollMutation.isPending} className="h-14 px-8 text-lg w-full sm:w-auto">
