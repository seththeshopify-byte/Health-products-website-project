...
router.get("/testimonials", async (req, res) => {
  try {
    ...
    const filtered = category ? testimonials.filter(t => t.category === category) : testimonials;
    res.json(filtered);
  } catch (err) {
...
router.post("/testimonials", requireAdmin, async (req, res) => {
  try {
    const { name, photoUrl, text, category } = req.body;
    if (!name || !text) { res.status(400).json({ error: "Missing required fields" }); return; }
    const [t] = await db.insert(testimonialsTable).values({ name, photoUrl: photoUrl ?? null, text, category: category ?? "product" }).returning();
